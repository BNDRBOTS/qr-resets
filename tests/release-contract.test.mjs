...
test("Railway config uses deterministic Docker builder and start-time volume initialization", () => {
  const railway = read("railway.toml");
...
test("Railway health gates real data readiness without making admin setup a deploy prerequisite", () => {
  ...
  assert.doesNotMatch(health, /dbReady && datasetReady && persistence && verifier && admin/);
});
