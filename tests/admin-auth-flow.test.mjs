...
test("a never-resolving authentication request times out instead of leaving the login spinner active", async () => {
  ...
  const component = read("src/components/bndr/admin-login-form.tsx");
  assert.match(component, /finally\s*\{\s*setLoading\(false\)/);
  assert.match(component, /Sign-in is temporarily unavailable\. Please try again\./);
});
