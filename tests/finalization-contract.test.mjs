...
test("standalone non-root views have explicit return paths", () => {
  ...
  for (const source of [login, admin, notFound, error]) {
    assert.match(source, /Back to Directory/);
  }
});

test("fresh site state remains BNDR/Directory", () => {
  const site = read("src/lib/use-site.ts");
  assert.match(site, /site: "bndr"/);
});


test("cool-blue dark palette is the global default and light/red remains mobile-accessible", () => {
