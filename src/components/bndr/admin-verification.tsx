                      ...
                      <div key={issue.id} className="rounded-md border border-border/60 p-2 text-xs">
                        ...
                        <div className="mt-2 flex gap-2">
                          <Button
                            ...
                            onClick={() => {
                              const confirmed = window.confirm(
                                "Accept this verifier issue resolution? This records the issue as accepted. It does not publish the resource.",
                              );
                              if (confirmed) issueMutation.mutate({ id: issue.id, reviewState: "accepted" });
                            }}
                          >
                            Accept
                          </Button>
                          <Button
                            ...
                            onClick={() => {
                              const confirmed = window.confirm(
                                "Dismiss this verifier issue? This records the issue as dismissed. It does not publish the resource.",
                              );
                              if (confirmed) issueMutation.mutate({ id: issue.id, reviewState: "dismissed" });
                            }}
                          >
                            Dismiss
                          </Button>
                          {issue.reviewState !== "unresolved" ? (
                            <Button
                              ...
                              onClick={() => {
                                const confirmed = window.confirm(
                                  "Reopen this issue as unresolved? Unresolved critical issues can block publication.",
                                );
                                if (confirmed) issueMutation.mutate({ id: issue.id, reviewState: "unresolved" });
                              }}
                            >
                              Reopen
                            </Button>
                ...
                {selected.publishState !== "published" ? (
                  <>
                    <Button
                      ...
                      onClick={() => {
                        const confirmed = window.confirm(
                          `Publish "${selected.sourceName}" to ResourceCite?\n\n` +
                            "This creates or updates the live canonical resource. Publication remains evidence-gated; a blocked gate will not be reported as success.",
                        );
                        if (confirmed) publishMutation.mutate(selected.id);
                      }}
                    >
                      Publish to ResourceCite
                    </Button>

                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button size="sm" variant="outline" disabled={reviewMutation.isPending}>
                      ...
                      <DropdownMenuContent align="end" className="min-w-56">
                        {selected.reviewState !== "reviewed" ? (
                          <DropdownMenuItem
                            onSelect={() => {
                              const confirmed = window.confirm(
                                "Mark this verification record reviewed? This records human review and may allow the publication gate to open. It does not publish the resource.",
                              );
                              if (confirmed) {
                        ...
                        {selected.publishState !== "excluded" ? (
                          <DropdownMenuItem
                            ...
                            onSelect={() => {
                              const confirmed = window.confirm(
                                "Exclude this record from publication? It will remain in verification history and can be moved back to held later; no canonical resource will be published by this action.",
                              );
                              if (confirmed) {
                                reviewMutation.mutate({
                                  id: selected.id,
                          ...
                          <DropdownMenuItem
                            onSelect={() => {
                              const confirmed = window.confirm(
                                "Move this excluded record back to held? It will remain unpublished and return to the review queue.",
                              );
                              if (confirmed) {
                                reviewMutation.mutate({
                                  id: selected.id,
                  ...
                  <p className="text-xs text-muted-foreground">
                    Published to ResourceCite. Manage the live row from the Resources tab.
                  </p>
                )}
                {!canPublish && selected.publishState === "held" ? (
                  <p className="w-full text-xs text-muted-foreground">
                    Held records require explicit human review before publication. Unresolved critical issues also block publication.
                  </p>
                ) : null}
              </div>
            </>
